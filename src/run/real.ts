...
import { applyGuardrails, GuardrailConfig, GuardrailContext, Mode } from "../execution/guardrails";
// DQN-REFAC TODO:
...
async function main() {
  ...
  const currentHour = Number(getArg("current-hour", String(new Date().getHours())));
  ...
  console.log({ mode, dailyTarget, peak, deltaMax, lambdaSpend, lagrangeStep, minHourly, maxHourly, canaries });

  // Minimal guardrails application: clamp a proposed hourly budget under daily cap
  const cfg: GuardrailConfig = { dailyBudgetTarget: dailyTarget, deltaMax, minHourly, maxHourly };
  ...
  if (mode === "pilot") {
    const res = applyGuardrails({ cfg, ctx, currentHourlyBudget: currentHourly, proposedHourlyBudget: proposedHourly });
    console.log("Guardrails:", { currentHourly, proposedHourly, projectedDailySpend: projected, allowedBudget: res.allowedBudget, reasons: res.reasons });
    console.log("NOTE: In pilot mode, the allowedBudget should be applied per canary before writes.")
  } else {
    console.log("Shadow mode: computing decisions only; no writes. You can still test guardrails with flags:");
    console.log("  --current-hourly --proposed-hourly --projected-daily-spend --min-hourly --max-hourly");
  }

  console.log("TODO: wire adapters (Meta/TikTok), Shopify datasource, guardrails application to agent actions, and reward");
}
