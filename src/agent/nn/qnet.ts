...
export class PlaceholderQNet implements QNet {
  ...
  trainOnBatch(_states: number[][], _actionsIdx: number[], _targets: number[]): number { return 0; }
...
export class QNetSimple implements QNet {
  ...
  private W2: number[][]; private b2: number[];
  private W3: number[][]; private b3: number[];
  private lr: number;

  constructor(private inputSize: number, private actionCount: number, hidden1 = 128, hidden2 = 64, lr = 1e-3) {
    this.lr = lr;
    const rand = (n: number, m: number) => Array.from({ length: n }, () => Array.from({ length: m }, () => (Math.random() - 0.5) * Math.sqrt(2 / (n + m))));
    const zeros = (n: number) => Array.from({ length: n }, () => 0);
    this.W1 = rand(hidden1, inputSize); this.b1 = zeros(hidden1);
    this.W2 = rand(hidden2, hidden1);  this.b2 = zeros(hidden2);
    this.W3 = rand(actionCount, hidden2); this.b3 = zeros(actionCount);
  }
  ...
  trainOnBatch(states: number[][], actionsIdx: number[], targets: number[]): number {
    ...
    const H1 = this.W1.length; const H2 = this.W2.length; const A = this.actionCount; const B = states.length;
    // Accumulate grads
    const gW1 = this.zerosLike(this.W1); const gb1 = new Array(H1).fill(0);
    const gW2 = this.zerosLike(this.W2); const gb2 = new Array(H2).fill(0);
    const gW3 = this.zerosLike(this.W3); const gb3 = new Array(A).fill(0);

    let lossSum = 0;
    for (let i = 0; i < B; i++) {
      const x = states[i]!; const aIdx = actionsIdx[i]!; const y = targets[i]!;
      // Forward with caches for this sample
      const z1 = this.affine(this.W1, this.b1, x); const a1 = z1.map(relu);
      const z2 = this.affine(this.W2, this.b2, a1); const a2 = z2.map(relu);
      const q = this.affine(this.W3, this.b3, a2);
      const qsa = q[aIdx]!;
      const diff = qsa - y;
      lossSum += diff * diff;
      // dL/dq is zero for all except action index
      const dQ = new Array(A).fill(0); dQ[aIdx] = 2 * diff / B; // average over batch
      // Backprop to W3, b3
      ...
      for (let j = 0; j < H2; j++) {
        const dz = dz2[j]!; const Wrow = this.W2[j]!;
    ...
    for (let i = 0; i < this.W1.length; i++) {
      for (let j = 0; j < this.W1[i]!.length; j++) this.W1[i]![j]! -= this.lr * clip(gW1[i]![j]!);
      this.b1[i]! -= this.lr * clip(gb1[i]!);
    }
    for (let i = 0; i < this.W2.length; i++) {
      for (let j = 0; j < this.W2[i]!.length; j++) this.W2[i]![j]! -= this.lr * clip(gW2[i]![j]!);
      this.b2[i]! -= this.lr * clip(gb2[i]!);
    }
    for (let i = 0; i < this.W3.length; i++) {
      for (let j = 0; j < this.W3[i]!.length; j++) this.W3[i]![j]! -= this.lr * clip(gW3[i]![j]!);
  ...
  copyFrom(source: QNet): void {
    ...
    this.W1 = s.clone2D(s.W1); this.b1 = s.b1.slice();
    this.W2 = s.clone2D(s.W2); this.b2 = s.b2.slice();
    this.W3 = s.clone2D(s.W3); this.b3 = s.b3.slice();
  }

  async save(path: string): Promise<void> {
    const payload = { W1: this.W1, b1: this.b1, W2: this.W2, b2: this.b2, W3: this.W3, b3: this.b3 };
  ...
  async load(path: string): Promise<void> {
    ...
    this.W1 = obj.W1; this.b1 = obj.b1; this.W2 = obj.W2; this.b2 = obj.b2; this.W3 = obj.W3; this.b3 = obj.b3;
  }
  ...
  private affineRelu(W: number[][], b: number[], x: number[], relu: (x: number) => number): number[] {
